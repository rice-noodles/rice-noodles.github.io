...
> redis 官网： https://redis.io/topics/introduction 

### 简单介绍

Redis：开源、免费、非关系型数据库、K-V数据库、内存数据库，支持持久化、事务和备份，集群(支持16个库)等高可用功能。并且性能极高(可以达到100000+的QPS)，易扩展，丰富的数据类型，所有操作都是单线程,原子性的。

> Redis 是基于内存操作，单线程的。CPU不是 Redis 的瓶颈，Redis 的瓶颈是机器的内存和网络带宽。
>
> Redis 将所有的数据全部放在内存中，使用单线程操作，减少了采用多线程时 CPU 上下文切换的时间消耗。
...
~~~shell
redis-cli	# 启动 Redis 服务后，使用该命令进入 Redis 客户端

select n	# 切换到第 n 个数据库
...
setex key times value	# 创建一个key，并且设置他的过期时间

mset key1 value1 key2 value2 	# 同时设置多个 key value

msetnx	# 同 setnx，msetnx 是原子操作

get key	# 获取 key 所对应的值
...
> **String应用场景**：
> 1、String通常用于保存单个字符串或JSON字符串数据，也可以是数字
...
~~~shell
lpush key value1 [value2]	# 从左侧插入，右边的先出,相当于压栈
...
eg:list1中元素1 2 3 4 5 ltrim list 1 2 3 list1剩余元素:3 4
...
> Jedis 是 Redis 官方推荐的 java 连接工具；是使用 java 操作 Redis 的中间件

~~~ java
Jedis jedis = new Jedis();	# 使用 jedis 操作 redis 的相关命令
jedis.set("key", "value");
~~~
...
> 在SpringBoot2.x之后，原来的 Jedis 被替换为了 lettuce
>
> jedis：采用直连，在多线程操作的时候是不安全的。若要避免线程不安全，则需要使用 jedis pool 连接池，像 BIO 模式
>
> lettuce：采用 netty，实例可以在多个线程中共享，不存在线程不安全的情况。可以减少线程数据，像 NIO 模式
...
+ Redis 的事务可以一次执行多个命令，其本质是一组命令的集合。

+ 一个事务中的所有命令都会被序列化，按顺序执行（队列）。

+ 一个队列中，一次性，顺序性，排他性执行一系列命令。

> **注意：**Redis 单条命令保证原子性，但是Redis 事务不保证原子性；Redis 事务没有隔离级别的概念
...
**命令入队：**将多个命令入队到事务中，接到这些命令不会立刻执行，而是放到等待执行的事务队列里面
...
+ **单独的隔离操作：**事务中的所有命令都会序列化，按顺序的执行。事务在等待执行的时候，不会被其他客户端发送来的命令请求打断

+ **没有隔离级别的概念：**队列中的所有命令没有提交 *exec* 之前都是不会被执行的

+ **不保证原子性：**Redis 中如果一条命令执行失败，其后的命令仍然会被执行，没有回滚
...
2. **放弃事务**
   ...
   get key_1	# 其返回的结果将是 nil，因为使用了 discard 命令取消事务
   ~~~

   

3. **编译时异常：**（代码有误）事务中的所有命令都不会被执行
   ...
   QUEUED
   127.0.0.1:6379> asddsa	# 输入错误命令
   (error) ERR unknown command `asddsa`, with args beginning with:
   127.0.0.1:6379> exec	# 由于命令错误，即出现编译时异常，所以事务中的所有命令都不执行
...
4. **运行时异常：**（1/0）队列中有问题的那一段命令不执行，其他命令可以执行。错误命令会抛出异常
   ...
   127.0.0.1:6379> set k1 v1
   QUEUED
   127.0.0.1:6379> incr k1	# 由于 k1 对应的值是 v1 字符串，是不能自增的，出现运行时异常
   QUEUED
   127.0.0.1:6379> set k2 v2
   QUEUED
   127.0.0.1:6379> set k3 v3
   QUEUED
   127.0.0.1:6379> exec	# 运行时异常，事务中错误的命令不执行，其他命令执行
   1) OK
...
+ watch 指令，类似乐观锁。
+ 如果 key 的值被修改了，那么**整个事务队列都不会被执行**，同时返回一个Nullmulti-bulk应答以通知调用者事务执行失败。

> 注意：**一旦执行了 exec或者 discard，之前加的所有监控锁都会被取消掉了。**

![1590644551348](C:\Users\huhan\AppData\Roaming\Typora\typora-user-images\1590644551348.png)

**转账实例：**

- 事务开启前，**被 watch 监控的值无改变**
  ...
  OK
  127.0.0.1:6379> watch money	# money 无变化，事务正常执行
  OK
  127.0.0.1:6379> multi
  OK
...
- 事务开启前，**被 watch 监控的值被修改**
  ...
  OK
  127.0.0.1:6379> set money 200	# money 的值发生改变
  OK
  127.0.0.1:6379> multi
  OK
  ...
  QUEUED
  127.0.0.1:6379> exec	# 由于被 watch 监控的值发生改变，所以事务不执行
  (nil)
...
- **unwatch**
  ...
  OK
  127.0.0.1:6379> unwatch	# 取消监控之后，事务正常执行
  OK
  127.0.0.1:6379> multi
  OK
